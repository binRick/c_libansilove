...
#define ANSI_SEQUENCE_MAX_LENGTH    18
    ...
    case STATE_SEQUENCE:
      ...
      for (ansi_sequence_loop = 0; ansi_sequence_loop < maxlength; ansi_sequence_loop++) {
        ...
        if (ansi_sequence_character == 'm') {
          ...
          struct StringFNStrings seqGrab_split = stringfn_split(seqGrab, ';');
          size_t                 rgb[5];
          options->truecolor = false;
          if (seqGrab_split.count == 5) {
            options->truecolor = true;
            rgb[0] = atoi(seqGrab_split.strings[2]);
            rgb[1] = atoi(seqGrab_split.strings[3]);
            rgb[2] = atoi(seqGrab_split.strings[4]);
            rgb[3] = atoi(seqGrab_split.strings[1]);
            rgb[4] = atoi(seqGrab_split.strings[0]);
            if (rgb[4] == 48) {
              background24 = RGB24(
                rgb[0],
                rgb[1],
            ...
            if (false) {
              fprintf(stderr, "=======================\n");
              fprintf(stderr, "24bit ANSI RGB!\n");
              fprintf(stderr, "       fg: %lu\n", foreground24);
              fprintf(stderr, "       bg: %lu\n", foreground24);
              fprintf(stderr, "ANSI RGB> type:%lu   |    r:%lu|g:%lu|b:%lu\n",
                      rgb[3],
                      rgb[0],
                      rgb[1],
                      rgb[2]
        ...
        if (ansi_sequence_character == 't') {
          ...
          fprintf(stderr, "pablo draw seq.....'%s'\n", seqGrab);
